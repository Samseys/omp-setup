...
Sono guardati anche i file del clone (`agent-config.json`, `mcp-subset.json`, `append-system.md`, `plugins.json`): un `capture` o un `git pull` risolvono la deriva senza toccare il `config.yml`, e senza quei file la riga resterebbe sull'avviso fino alla riapertura proprio dopo averla risolta.
